...
async function getMovie(id) {
    ...
    const [moviesRes, likesRes, hasLikedRes] = await Promise.all(requests);
    ...
    if (userData !== null) {
        if (userData.id === movie._ownerId) {
            ...
            if (hasLiked.length > 0) {
                controls.appendChild(e('a', { className: 'btn-primary', href: '#', onClick: onUnlike }, 'Unlike'));
            } else {
                controls.appendChild(e('a', { className: 'btn-primary', href: '#', onClick: onLike }, 'Like'));
            }
        }
    }
